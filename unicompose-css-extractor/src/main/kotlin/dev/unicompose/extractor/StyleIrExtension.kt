...
internal class StyleIrExtension(private val outputDir: String? = null) : IrGenerationExtension {
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        ...
        val totalSites = visitor.discovered.size
        val emissions = visitor.emissions
        if (totalSites > 0) {
            System.err.println(
        ...
        if (totalSites > 0) {
            runCatching {
                ...
                val css = buildString {
                    appendLine("/* unicompose-generated.css — auto-generated by unicompose-css-extractor.")
                    appendLine(" * Module: ${moduleFragment.name.asString()}.")
                    appendLine(" * Static extraction coverage: ${emissions.size}/$totalSites Style() sites.")
                    appendLine(" */")
                    for ((cls, e) in unique) {
                        appendLine(".$cls{${e.cssBody}}")
                    }
                }
                cssOut.writeText(css)
                System.err.println("  → wrote ${unique.size} unique classes to ${cssOut.absolutePath}")
            }
        }
        if (visitor.dump.isNotEmpty()) {
            runCatching {
                ...
                out.writeText(
                    "module: ${moduleFragment.name.asString()} (${totalSites} sites, ${emissions.size} static)\n" +
...
private class StyleVisitor : IrVisitorVoid() {
    ...
    val emissions: MutableList<CssEmitter.Emission> = mutableListOf()
    ...
    private fun dumpArguments(call: IrConstructorCall, indent: String) {
        ...
        val resolved = mutableMapOf<String, Evaluator.V>()
        ...
        params.forEachIndexed { idx, param ->
            ...
            if (evaluated == null) {
                ...
                resolved[param.name.asString()] = evaluated
                dump += "$indent${param.name.asString()} = $evaluated"
            }
        }
        if (allEvaluable && resolved.isNotEmpty()) {
            val emission = CssEmitter.emit(resolved)
            if (emission != null) {
                emissions += emission
                dump += "$indent[STATIC — extractable] → .${emission.className} { ${emission.cssBody} }"
            } else {
                dump += "$indent[STATIC but no rules emitted — none of the resolved args mapped to CSS]"
            }
        } else {
            dump += "$indent[DYNAMIC — runtime]"
        }
    }
